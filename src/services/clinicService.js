    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.name || !data.descriptionHTML || !data.descriptionMarkdown || !data.descriptionHTMLEnglish || !data.descriptionMarkdownEnglish || !data.nameEnglish || !data.address || !data.addressEnglish
                ...
                // await db.Clinic.create({ name: data.name });
...
let getTop_Clinic_Facility_Home = (limitInput) => {
    return new Promise(async (resolve, reject) => {
        
        try {
            let clinicsFromDatabase = await db.Clinic.findAll({
                limit: limitInput,
                // where: {
                //     roleId: 'R2'
                // },
            ...
            resolve({
                message: 'Get Top clinic successfully !',
                errCode: 0,
                data: clinicsFromDatabase
            });
...
module.exports = {
    ...
    getTop_Clinic_Facility_Home  :getTop_Clinic_Facility_Home
}