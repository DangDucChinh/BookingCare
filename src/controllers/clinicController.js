...
let handle_Get_Medical_Facility_Home = async (req, res) => {
    let limit = req.query.limit; 
    if (!limit) limit = 10;

    try {
        let respone = await clinicService.getTop_Clinic_Facility_Home(+limit);

        return res.status(200).json(respone);

    } catch (error) {
        console.log('Lỗi tại clinicController  : ', error);
        return res.status(200).json({
            errCode: -1,
            message: 'Lỗi from server, fix tại clinicController '
        });
    }
}

module.exports = {
    handle_Get_Medical_Facility_Home: handle_Get_Medical_Facility_Home,
